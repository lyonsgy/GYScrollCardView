...
- (void)setupScrollLineAndCover {
    ...
    if (self.coverLayer) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            if (!self.segmentStyle.coverWidth) {
                self.coverLayer.frame = CGRectMake(coverX - xGap, coverY, coverW + wGap, coverH);
            }else{
                self.coverLayer.frame = CGRectMake(coverX - self.segmentStyle.coverWidth, coverY, coverW + 2*self.segmentStyle.coverWidth, coverH);
            }
        } else {
            if (!self.segmentStyle.coverWidth) {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                coverW = [self.titleWidths[_currentIndex] floatValue] + 2*self.segmentStyle.coverWidth;
                coverX = (firstLabel.zj_width - coverW) * 0.5;
            }
        ...
        if (weakSelf.coverLayer) {
            if (weakSelf.segmentStyle.isScrollTitle) {
                if (!self.segmentStyle.coverWidth) {
                    ...
                    weakSelf.coverLayer.zj_x = currentTitleView.zj_x - self.segmentStyle.coverWidth;
                    weakSelf.coverLayer.zj_width = currentTitleView.zj_width + 2*self.segmentStyle.coverWidth;
                }
            } else {
                if (!self.segmentStyle.coverWidth) {
                    if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    CGFloat coverW = [self.titleWidths[_currentIndex] floatValue] + 2*self.segmentStyle.coverWidth;
                    CGFloat coverX = currentTitleView.zj_x + (currentTitleView.zj_width - coverW) * 0.5;
                    weakSelf.coverLayer.zj_x = coverX;
                    weakSelf.coverLayer.zj_width = coverW;
                }
    ...
    if (self.scrollLine) {
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (!self.segmentStyle.scrollLineWidth) {
                ...
                self.scrollLine.zj_x = oldTitleView.zj_x+xDistance*progress+(currentTitleView.zj_width - self.segmentStyle.scrollLineWidth) * 0.5;
    ...
    if (self.coverLayer) {
        if (self.segmentStyle.isScrollTitle) {
            if (!self.segmentStyle.coverWidth) {
                ...
                self.coverLayer.zj_x = oldTitleView.zj_x + xDistance * progress - self.segmentStyle.coverWidth;
                self.coverLayer.zj_width = oldTitleView.zj_width + wDistance * progress + 2*self.segmentStyle.coverWidth;
            }
        } else {
            if (!self.segmentStyle.coverWidth) {
                if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                CGFloat oldCoverW = [self.titleWidths[oldIndex] floatValue] + 2*self.segmentStyle.coverWidth;
                CGFloat currentCoverW = [self.titleWidths[currentIndex] floatValue] + 2*self.segmentStyle.coverWidth;
                wDistance = currentCoverW - oldCoverW;
                CGFloat oldCoverX = oldTitleView.zj_x + (oldTitleView.zj_width - oldCoverW) * 0.5;
                CGFloat currentCoverX = currentTitleView.zj_x + (currentTitleView.zj_width - currentCoverW) * 0.5;
                xDistance = currentCoverX - oldCoverX;
